...
void fast10_avx2_gather4x8(uint8_t* data, uint32_t width, uint32_t height, uint32_t row_stride, std::vector<ImageRef>& corners, const int barrier)
{
    ...
    const __m256i offsets = _mm256_set_epi32(row_stride * 7, row_stride * 6, row_stride * 5, row_stride * 4, row_stride * 3, row_stride * 2, row_stride, 0);

    for (int y = 3; y < yend; y += 8)
    {
        for (int x = 32; x < xend; x += 4)
        {
#if COUNT_CHECKS
            check[0]++;
#endif
            ...
            __m256i lo, hi;
            {
                ...
                const __m256i v = _mm256_i32gather_epi32((const int*)p, offsets, 1);
            ...
            uint64_t ans_b, ans_e;
            {
                ...
                __m256i top = _mm256_i32gather_epi32((const int*)(p - stride), offsets, 1);
                __m256i bottom = _mm256_i32gather_epi32((const int*)(p + stride), offsets, 1);
            ...
            uint64_t ans_m, ans_p, possible;
            {
                ...
                __m256i ul = _mm256_i32gather_epi32((const int*)(p - 2 - 2 * row_stride), offsets, 1);
                __m256i lr = _mm256_i32gather_epi32((const int*)(p + 2 + 2 * row_stride), offsets, 1);
            ...
            uint64_t ans_o, ans_n;
            {
                ...
                __m256i ll = _mm256_i32gather_epi32((const int*)(p - 2 + 2 * row_stride), offsets, 1);
                __m256i ur = _mm256_i32gather_epi32((const int*)(p + 2 - 2 * row_stride), offsets, 1);
            ...
            uint64_t ans_h, ans_k;
            {
                ...
                __m256i left = _mm256_i32gather_epi32((const int*)(p - 3), offsets, 1);
                __m256i right = _mm256_i32gather_epi32((const int*)(p + 3), offsets, 1);
            ...
            uint64_t ans_a, ans_c;
            {
                ...
                __m256i a = _mm256_i32gather_epi32((const int*)(p - 1 - stride), offsets, 1);
                __m256i c = _mm256_i32gather_epi32((const int*)(p + 1 - stride), offsets, 1);
            ...
            {
                __m256i d = _mm256_i32gather_epi32((const int*)(p - 1 + stride), offsets, 1);
                __m256i f = _mm256_i32gather_epi32((const int*)(p + 1 + stride), offsets, 1);
            ...
            uint64_t ans_g, ans_i;
            {
                ...
                __m256i g = _mm256_i32gather_epi32((const int*)(p - 3 - row_stride), offsets, 1);
                __m256i ii = _mm256_i32gather_epi32((const int*)(p - 3 + row_stride), offsets, 1);
            ...
            uint64_t ans_j, ans_l;
            {
                ...
                __m256i jj = _mm256_i32gather_epi32((const int*)(p + 3 - row_stride), offsets, 1);
                __m256i l = _mm256_i32gather_epi32((const int*)(p + 3 + row_stride), offsets, 1);
